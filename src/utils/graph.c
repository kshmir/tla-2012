...
int graph_arc_comparer(void * a1, void * a2) {
...
static int compare_data(void * id, void * data_struct) {
	return comparer_storage[c_index](id,data_struct);
}



graph_node node_init(void * key, void * value){
...
int graph_add_node(graph g, void * key, void * value){
...
int graph_add_arc(graph g, void * from, void * to, int weight){
	pthread_mutex_lock(g->mutex);
	c_index = g->comparer_index;
...
int graph_remove_arc(graph g, void * from, void * to){
	pthread_mutex_lock(g->mutex);
	c_index = g->comparer_index;
...
graph_node graph_get_node(graph g, void * key){
	pthread_mutex_lock(g->mutex);
	c_index = g->comparer_index;
	graph_node n = map_get(g->nodes, key);
	pthread_mutex_unlock(g->mutex);
...
void * graph_node_key(graph_node n){
	return n->key;
}
// Devuelve el valor del nodo
void * graph_node_value(graph_node n){
	if (n == NULL) {
		return NULL;
	}
	return n->value;
